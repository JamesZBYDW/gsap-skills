...
import { advanceRequest } from '@/server/lifecycle';
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  return handle(async () => {
    ...
    if (!parsed.success) return errorJson(parsed.error.issues[0]?.message ?? 'Invalid status.', 400);
    await advanceRequest(id, parsed.data.status, user.id, clientIp(req));
    return json({ ok: true });
  });
}
